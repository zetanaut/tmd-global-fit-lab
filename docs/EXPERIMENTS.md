...
| W09 PV17 exact-data baseline | Scientific/code agent, then one GPU | Row accounting passed: 21,951 candidates -> 8,283 selected rows -> 8,059 effective observations after 224 COMPASS constraints. Freeze the modern covariance contract; construct and validate the present N3LL matched operator; then fit the current DNN sequentially on the RTX 4090 and assess grouped holdouts/TMD stability. The historical NLL fit is not rerun, MAPTMD22 is not a required fallback, and no 192-update cap applies. |
