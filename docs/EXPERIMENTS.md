...
| W09 historical closure baseline | Scientific/code agent, then one GPU | Row accounting passed: 21,951 candidates -> 8,283 selected rows -> 8,059 effective points after 224 COMPASS constraints. Recover the remaining exact PV17 final-run identity; replay its 11-parameter result (using GPU if useful); distill its TMDs into the DNN; then run same-data paired fits and grouped holdouts. MAPTMD22 is fallback only after exact artifact binding. No DNN optimization before H0/H1. |
